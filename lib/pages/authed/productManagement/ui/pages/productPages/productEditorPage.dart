// ignore_for_file: no_leading_underscores_for_local_identifiers, non_constant_identifier_names, use_build_context_synchronously, deprecated_member_use
...
class _ProductEditorPageState extends State<ProductEditorPage> {
  ...
  Future<void> _saveB_callback() async {
    ...
    if (_isUpdating) {
      ...
      context
          .read<ProductBloc>()
          .add(InsertProduct(product: product, category: _category));
    }

    locator<AppDialogs>().disposeAnyActiveDialogs();

    GoRouter.of(context).go(
      "${AppRoutes.productManagement}/${AppRoutes.product}",
    );
