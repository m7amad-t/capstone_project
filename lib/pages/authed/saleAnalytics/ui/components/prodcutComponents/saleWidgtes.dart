...
class ProductSaleSection extends StatefulWidget {
  final ProductModel product;
  const ProductSaleSection({super.key, required this.product});

  @override
  State<ProductSaleSection> createState() => _ProductSaleSectionState();
}

class _ProductSaleSectionState extends State<ProductSaleSection> {
  final _service = AnalyticsService();

  late ProductSaleModel saleData;
  bool isLoading = true;
  ...
  _laodData() async {
    ...
    final res = await _service.getProductSale(
      _selectedDateRange,
      widget.product.id,
    );
  ...
  Widget _changeDateButton(TextTheme textStyle, BuildContext contextee) {
    ...
    return tableKit.table(
      ...
      rows: [
        for (final row in saleData.months)
          tableKit.row(
            cells: [
              tableKit.cell(
  ...
  List<CartesianSeries<ProductMonthSaleModel, String>> _buildAreaSeries(
      BuildContext context) {
    return <CartesianSeries<ProductMonthSaleModel, String>>[
      SplineAreaSeries<ProductMonthSaleModel, String>(
        dataSource: saleData.months,
        xValueMapper: (data, index) => data.month.name(context),
        yValueMapper: (data, index) => totalFormatted(data.sale),
